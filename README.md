...
Clinical Assistant реализован как цифровая образовательная система с несколькими связанными компонентами:

- веб-интерфейс Clinical Assistant;
- интерактивное обучение с учебными наборами;
- клинические кейсы с уровнями сложности;
- AI-чат для образовательных вопросов;
- отдельный учебный трекер для организации учебного процесса;
- административная панель для управления пользователями и учебными материалами;
- импорт учебных вопросов из Excel;
- Windows-приложение для офлайн-прохождения модуля «Обучение».

Проект развёрнут на выделенном сервере и доступен в виде работающей веб-системы.

Разработка велась с использованием OpenCode.

---

## Результат проекта

В результате создана единая образовательная система, объединяющая:

**Обучение → Клинические кейсы → AI-чат → Учебный трекер → Windows Offline**

Система поддерживает как онлайн-формат работы через веб-интерфейс, так и локальное прохождение учебного модуля на Windows без подключения к интернету.

Проект представляет собой не отдельную веб-страницу, а связанный набор образовательных интерфейсов, учебных материалов и инструментов контроля обучения.
